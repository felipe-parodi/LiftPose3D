...
def plot_3d_graph(pos, ax, color_edge = 'k', style = '-'):
    ...
    for i, j in enumerate(G.edges()): 
        ...
        if xid is not None and yid is not None:
            ...
            z = 2461.8179
            #print(z)
            ax.plot(x, y, -z, style, c='black', alpha=1.0, linewidth = 2) 
...
data_dir = '/data/LiftFly3D/flylimbtracker/network' #data directory
template_dir = '/data/LiftFly3D/optobot/network/'
        
#predictions
data = torch.load(data_dir + '/test_results.pth.tar')
out_offset, inp_offset = pickle.load(open('./flylimbtracker/joint_locations.pkl','rb'))
...
with writer.saving(fig, "prediction_cams.mp4", 100):
    for t in tqdm(range(n_frames)):
        ...
        ax.set_axis_off()
        plt.savefig('./flylimbtracker/out/{:03d}.jpg'.format(t))
