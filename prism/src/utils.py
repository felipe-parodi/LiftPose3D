...
def select_best_data(bottom, side, th1, th2, leg_tips):
    ...
    mask = (
        (((side_L_lk > th1).sum(1) == 3) & ((side_R_lk > th1).sum(1) == 0))
    ...
    diff_L = np.abs(
        bottom.loc[:, (leg_tips[:3], "x")].values
        - side.loc[:, (leg_tips[3:], "x")].values
    )
    diff_R = np.abs(
        bottom.loc[:, (leg_tips[3:], "x")].values
        - side.loc[:, (leg_tips[:3], "x")].values
    ...
    assert (
        side.shape[0] == bottom.shape[0]
    ), "Number of rows must match in filtered data!"
