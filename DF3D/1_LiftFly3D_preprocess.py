...
def main():
    ...
    # 3D ground truth
    train_set, test_set, data_mean, data_std, targets_3d, vis = read_3d_data(
    ...
    torch.save(
        {"mean": data_mean, "std": data_std, "targets_3d": targets_3d},
        out_dir + "/stat_3d.pth.tar",
    )

    # HG prediction (i.e. deeplabcut or similar)
    train_set, test_set, data_mean, data_std, targets_2d = read_2d_predictions(
        actions, data_dir, rcams, target_sets, ref_points, vis
    )

    torch.save(train_set, out_dir + "/train_2d.pth.tar")
    torch.save(test_set, out_dir + "/test_2d.pth.tar")
    torch.save(
        {"mean": data_mean, "std": data_std, "targets_2d": targets_2d},
    ...
    for fly in flies:
        for action in actions:
            ...
            fname = [
                file
                for file in fnames
                if ("Fly" + str(fly) + "_" in file) and (action in file)
            ]
        ...
        for action in actions:
            ...
            for fname_ in fname:
                ...
                poses_cam = np.reshape(
                    poses_cam, (poses.shape[1], poses.shape[2] * poses.shape[3])
                )
